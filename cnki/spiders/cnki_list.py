...
class CnkiListSpider(scrapy.Spider):
    ...
    page=1
    allPage=1
    allowed_domains = ['http://www.cnki.net','kns.cnki.net','http://www.baidu.com','www.baidu.com','search.cnki.net']
    ...
    def spide_list(self, response):
        ...
        yield item
        keyword = urllib.parse.quote(self.getKeyWord())
        url="http://kns.cnki.net/kns/brief/brief.aspx?timestamp="+str(time.time())+"&pagename=ASP.brief_default_result_aspx&dbPrefix=SCDB&dbCatalog=%e4%b8%ad%e5%9b%bd%e5%ad%a6%e6%9c%af%e6%96%87%e7%8c%ae%e7%bd%91%e7%bb%9c%e5%87%ba%e7%89%88%e6%80%bb%e5%ba%93&ConfigFile=SCDBINDEX.xml&research=off&t=1506415981955&keyValue="+keyword+"S=1"
        self.getCookie(response)
        ...
        if "vericode.aspx" in response.url:
            ...
            for passage in pass_list:
                try:
                    ...
                    item['url'] = passage.xpath('./td[2]/a/@href').extract()[0]
                    name =passage.xpath("./td[2]/a").extract()[0]
                    item['name'] = self.getName(name)
                    item['source']= passage.xpath("./td[4]/a/text()").extract()[0]
                    item['pubdata'] = passage.xpath("./td[5]/text()").extract()[0].strip()
                    cite=passage.xpath("./td[7]/span/a")
                    if len(cite):
                        cite_url=cite.xpath("./@onclick").extract()[0]
                        item['cite'] = cite.xpath("./text()").extract()[0]
                    else :
                        item['cite']=0
                    item['download'] = passage.xpath("./td[8]/span/a/text()").extract()[0]
                    item['type']=1
                    yield item
                except:
                    pass
    ...
    def getPageUrl(self,response):
        ...
        if self.page==1:
            ...
            url = "http://kns.cnki.net/kns/brief/brief.aspx?curpage=" + str(self.page) + "&RecordsPerPage=50&QueryID=0&ID=&turnpage=1&tpagemode=L&dbPrefix=SCDB&Fields=&DisplayMode=listmode&PageName=ASP.brief_default_result_aspx&sKuaKuID=0"
        self.getCookie(response)
